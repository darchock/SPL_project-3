...
private:
    ...
    // name of user responsible of sending this Event
    std::string username;

public:
    Event(std::string name, std::string team_a_name, std::string team_b_name, int time, std::map<std::string, std::string> game_updates, std::map<std::string, std::string> team_a_updates, std::map<std::string, std::string> team_b_updates, std::string discription, std::string username);
    Event(const std::string & frame_body);
    Event();
    Event(std::string team_a, std::string team_b);
    virtual ~Event();
